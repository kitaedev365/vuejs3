...
public class DataroomController {
    ...
    public ResponseEntity<Dataroom> getDataroomById(@PathVariable(name = "dno") int dno) {
        Optional<Dataroom> dataroom = dataroomService.getDataroomById(dno);
    ...
    public ResponseEntity<Dataroom> updateDataroom(@PathVariable(name = "dno") int dno,
    		@RequestParam("title") String title,
            @RequestParam("content") String content,
            @RequestParam("author") String author,
            @RequestParam("datafile") MultipartFile datafile) throws IOException {
        deleteFile(dataroomService.getDataroomById(dno).orElseThrow().getDatafile());
        ...
        return ResponseEntity.ok(dataroomService.updateDataroom(dno, updatedDataroom));
    }

    @DeleteMapping("/delete/{dno}")
    public ResponseEntity<Void> deleteDataroom(@PathVariable(name = "dno") int dno) {
        deleteFile(dataroomService.getDataroomById(dno).orElseThrow().getDatafile());
        dataroomService.deleteDataroom(dno);
