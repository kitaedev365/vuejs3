...
public class CartController {
    ...
    @PostMapping("/add")
    public ResponseEntity<?> addToCart(@RequestBody RequestCartDto cartDto) {
        try {
            // CartDto에서 User와 Product 정보가 제대로 포함되었는지 확인
            if (cartDto.getUsername() == null || cartDto.getId() == null) {
    ...
    public ResponseEntity<List<ResponseCartDto>> getUserCart(@PathVariable("username") String username) {
        try {
            List<ResponseCartDto> cartList = cartService.getUserCart(username);
